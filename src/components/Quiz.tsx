...
export function Quiz({ quizId, user, questions, onFinish, onHome }: QuizProps) {
  ...
  { qi: qi + 1, correct: ok },
    ...
    {done && <div className={`fb ${isRight ? "ok" : "bad"}`}><div className="fb-v">{isRight ? "🎉 Excellent!" : "💡 Not quite."}</div>{!isRight && <div className="fb-ca">Correct answer is option {letters[q.ans]}</div>}<div className="fb-ex"><div>Go beyond the question.</div><div>Discover more with SPAF Masterclass 2026</div></div><button className="btn-next" onClick={next}>{qi + 1 >= questions.length ? "📋 See My Results" : "Next Question"}</button></div>}
  </div></div>;
}
